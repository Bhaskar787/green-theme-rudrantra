...
export function Navbar() {
  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
    <Link href={href} className="relative py-2 text-sm font-heading font-medium text-cream hover:text-gold-bright transition-colors group whitespace-nowrap shrink-0">
      {children}
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-2.5 sm:px-4 md:px-8 flex items-center justify-between h-16 sm:h-20 gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-4 shrink-0 min-w-0">
          ...
          <Link href="/" className="flex items-center gap-2 sm:gap-3 group min-w-0">
            <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-full border border-gold overflow-hidden flex items-center justify-center p-1 relative shrink-0">
            ...
            <span className="hidden min-[420px]:block font-display text-lg sm:text-2xl font-bold tracking-widest text-gold text-gold-gradient whitespace-nowrap truncate">
              Rudrantra
            </span>
          </Link>
        </div>

        {/* CENTER (Desktop) — only renders once there's real room, so it can't collide with the right cluster */}
        <nav className="hidden xl:flex items-center justify-center gap-4 2xl:gap-6 flex-1 min-w-0 overflow-hidden">
          ...
          <Link href="#" className="text-sm font-heading font-medium text-cream hover:text-gold-bright transition-colors py-2 flex items-center gap-2 shrink-0 whitespace-nowrap">
            Shravan
            <span className="text-[10px] tracking-widest uppercase bg-crimson text-white px-2 py-0.5 rounded-full font-bold shadow-[0_0_8px_rgba(139,26,26,0.5)] shrink-0">Event</span>
          </Link>
        </nav>

        {/* RIGHT */}
        <div className="flex items-center justify-end gap-2 sm:gap-3 md:gap-5 shrink-0">
          <div className="hidden md:flex items-center gap-1 mr-1 text-sm font-heading font-medium text-gold shrink-0">
            <span>₹</span>
          ...
          <button onClick={() => setIsSearchOpen(true)} className="p-1.5 sm:p-2 text-gold hover:text-gold-bright transition-colors rounded-full hover:bg-forest-light shrink-0">
            <Search className="w-[18px] h-[18px] sm:w-5 sm:h-5" />
          </button>

          <button className="hidden sm:block p-2 text-gold hover:text-gold-bright transition-colors rounded-full hover:bg-forest-light shrink-0">
            <User className="w-5 h-5" />
          </button>

          <button onClick={() => setIsWishlistOpen(true)} className="p-1.5 sm:p-2 text-gold hover:text-gold-bright transition-colors relative rounded-full hover:bg-forest-light shrink-0">
          ...
          <button onClick={() => setIsCartOpen(true)} className="p-1.5 sm:p-2 text-gold hover:text-gold-bright transition-colors relative rounded-full hover:bg-forest-light shrink-0">
