...
export function Navbar() {
  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
    <Link href={href} className="relative py-2 text-sm font-heading font-medium text-cream hover:text-gold-bright transition-colors group">
      {children}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between h-20">
        ...
        <nav className="hidden lg:flex items-center justify-center gap-8 flex-1">
          ...
          <div className="relative group cursor-pointer py-2">
            <span className="text-sm font-heading font-medium text-cream group-hover:text-gold-bright transition-colors flex items-center gap-1">
              Rudraksha
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300 translate-y-2 group-hover:translate-y-0">
              <div className="bg-forest-deep border border-gold/30 shadow-[0_10px_30px_rgba(0,0,0,0.5)] rounded-xl p-2 w-56 flex flex-col gap-1 relative overflow-hidden">
                ...
                <Link href="#" className="relative z-10 px-4 py-3 text-sm font-heading text-cream hover:bg-forest-light hover:text-gold rounded-lg transition-colors border-l-2 border-transparent hover:border-gold">Rudraksha Beads</Link>
                <Link href="#" className="relative z-10 px-4 py-3 text-sm font-heading text-cream hover:bg-forest-light hover:text-gold rounded-lg transition-colors border-l-2 border-transparent hover:border-gold">Rudraksha Mala</Link>
                <Link href="#" className="relative z-10 px-4 py-3 text-sm font-heading text-cream hover:bg-forest-light hover:text-gold rounded-lg transition-colors border-l-2 border-transparent hover:border-gold">Rudraksha Bracelet</Link>
                <Link href="#" className="relative z-10 px-4 py-3 text-sm font-heading text-cream hover:bg-forest-light hover:text-gold rounded-lg transition-colors border-l-2 border-transparent hover:border-gold">Combination & Kawach</Link>
              </div>
            </div>
          </div>
          ...
          <Link href="#" className="text-sm font-heading font-medium text-cream hover:text-gold-bright transition-colors py-2 flex items-center gap-2">
            Shravan
