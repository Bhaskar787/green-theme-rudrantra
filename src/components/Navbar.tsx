...
export function Navbar() {
  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
    <Link href={href} className="relative py-2 text-[11px] sm:text-xs md:text-sm font-heading font-medium text-cream hover:text-gold-bright transition-colors group whitespace-nowrap shrink-0">
      {children}
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 md:px-8 flex items-center justify-between h-16 sm:h-20 gap-2">
        ...
        <div className="flex items-center justify-end gap-1 sm:gap-2 md:gap-4 shrink-0">
          <div className="hidden md:flex items-center gap-1 text-[10px] sm:text-xs font-heading font-medium text-gold shrink-0">
            <span>₹</span>
            <select className="bg-transparent border-none focus:outline-none cursor-pointer appearance-none text-gold hover:text-gold-bright pr-3 bg-forest-deep">
              <option>INR</option>
              <option>USD</option>
            </select>
          </div>
          ...
          <button className="hidden sm:block p-1.5 sm:p-2 text-gold hover:text-gold-bright transition-colors rounded-full hover:bg-forest-light shrink-0">
          ...
          <button onClick={() => setIsWishlistOpen(true)} className="p-1.5 sm:p-2 text-gold hover:text-gold-bright transition-colors relative rounded-full hover:bg-forest-light shrink-0">
            ...
            {wishlistCount > 0 && (
              <span className="absolute top-0 right-0 w-3.5 h-3.5 sm:w-4 sm:h-4 bg-crimson text-white text-[8px] sm:text-[10px] font-bold flex items-center justify-center rounded-full border border-forest-deep">
                {wishlistCount}
              </span>
            )}
          </button>
          ...
          <button onClick={() => setIsCartOpen(true)} className="p-1.5 sm:p-2 text-gold hover:text-gold-bright transition-colors relative rounded-full hover:bg-forest-light shrink-0">
            ...
            {cartCount > 0 && (
              <span className="absolute top-0 right-0 w-3.5 h-3.5 sm:w-4 sm:h-4 bg-gold text-forest-deep text-[8px] sm:text-[10px] font-bold flex items-center justify-center rounded-full border border-forest-deep">
                {cartCount}
              </span>
            )}
          </button>
