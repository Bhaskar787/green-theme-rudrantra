...
export function SacredCollections() {
  return (
    <section className="py-14 sm:py-20 md:py-24 bg-forest relative overflow-hidden">
      {/* OM Section Divider */}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-8 mb-10 sm:mb-16">
          <div>
            <span className="text-[10px] sm:text-xs font-heading font-bold uppercase tracking-[0.2em] text-gold bg-gold/5 border border-gold/20 px-3.5 sm:px-4 py-1.5 rounded-full inline-block mb-4 sm:mb-6">
              ॐ Categories
            </span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display text-gold-gradient tracking-tight">Shop by Category</h2>
          </div>
          <p className="text-cream-soft font-body text-sm sm:text-base max-w-md opacity-80 border-l border-gold/30 pl-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 md:gap-6">
          {collections.map((item, i) => (
            <a key={i} href="#" className="group flex flex-col gap-2.5 sm:gap-3.5 md:gap-5">
              <div className="aspect-square rounded-xl sm:rounded-2xl overflow-hidden border border-gold/20 relative shadow-lg group-hover:border-gold/70 group-hover:shadow-sacred-glow group-hover:-translate-y-1.5 transition-all duration-500 ring-1 ring-inset ring-white/5">
                <img 
                  src={item.image} 
                  alt={item.name} 
                ...
                {/* Corner index number */}
                <div className="absolute top-2 left-2 sm:top-3 sm:left-3 w-5 h-5 sm:w-7 sm:h-7 rounded-full border border-gold/40 bg-forest-deep/60 backdrop-blur-sm flex items-center justify-center text-gold text-[8px] sm:text-[10px] font-heading font-bold tracking-wide">
              ...
              <span className="font-heading font-semibold text-center text-[11px] sm:text-sm md:text-base text-cream group-hover:text-gold transition-colors tracking-wide">
                {item.name}
              </span>
            </a>
          ))}
          ...
          <a href="#" className="group flex flex-col gap-2.5 sm:gap-3.5 md:gap-5">
            <div className="aspect-square rounded-xl sm:rounded-2xl border border-gold/30 bg-forest-deep flex flex-col items-center justify-center gap-3 sm:gap-4 transition-all duration-500 group-hover:border-gold group-hover:shadow-sacred-glow group-hover:-translate-y-1.5 relative overflow-hidden ring-1 ring-inset ring-white/5">
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full border border-gold/40 flex items-center justify-center text-gold group-hover:bg-gold group-hover:text-forest transition-colors relative z-10">
