...
const tabs = [
  ...
  },
  {
    ...
    detail: 'Invite abundance and financial clarity into daily life.',
  ...
  },
  {
    ...
    detail: 'Shield against negative energy and instability. ',
  ...
  },
  {
    ...
    detail: 'Restore balance in relationships and the home. ',
  ...
  },
  {
    ...
    detail: 'Support physical vitality and calm the nervous system.',
    image: 'https://himalayarudraksh.online/cdn/shop/files/1-13-mukhi-shiv-shakti-rudraksha-mala-nepal-origin-499218.png?v=1750001216&width=3840',
  },
];
  ...
  return (
    <>
      ...
      <div className="text-center px-4 mb-10 sm:mb-14 md:mb-16">
        ...
        <p className="text-cream-soft/70 font-body text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
...
function IntentionScrollFlow() {
  ...
  return (
    <div ref={containerRef} className="max-w-3xl mx-auto px-4 sm:px-6 pb-16 sm:pb-24 relative">
      <div className="absolute top-1 bottom-1 w-px bg-gold/20" style={{ left: '19px' }} aria-hidden>
          ...
          >
            <span
              className={`inline-block mb-2 px-3 py-1 rounded-full text-xs font-heading font-bold uppercase tracking-widest transition-colors duration-300 ${
                isReached ? 'bg-gold text-forest-deep' : 'bg-forest-mid text-cream/50'
              }`}
            >
              Step {i + 1}
            </span>
            <h3
              className={`font-display text-xl sm:text-2xl md:text-3xl mb-2 transition-colors duration-300 ${
                isReached ? 'text-cream' : 'text-cream/40'
              }`}
            >
              {tab.label}
            </h3>
            <p className="text-cream-soft/70 font-body text-sm sm:text-base leading-relaxed mb-5 sm:mb-6 max-w-lg">
              {tab.detail}
            </p>

            <div className="relative w-full aspect-[4/5] sm:aspect-[16/9] md:aspect-[16/10] rounded-2xl overflow-hidden border border-gold/20 shadow-xl">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-7">
                <h4 className="font-display text-xl sm:text-2xl md:text-3xl text-cream mb-1">{tab.label}</h4>
                <p className="text-cream-soft/70 font-body text-xs sm:text-sm max-w-md">{tab.desc}</p>
              </div>
            </div>

            <a
              href="#"
              className="inline-flex items-center gap-2 mt-5 sm:mt-6 px-5 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-gold to-gold-soft text-forest-deep rounded-full font-heading font-bold uppercase tracking-widest text-xs hover:shadow-sacred-glow transition-all"
            >
          ...
          <div className="flex flex-row items-center gap-16 xl:gap-24 w-full">
            ...
            <div className="flex-1 max-w-xl w-full">
              <div className="flex gap-6">
                <div className="relative w-px bg-gold/20 shrink-0" aria-hidden>
                  <div
                      ...
                      >
                        ...
                        <h3
                          className={`font-display text-xl xl:text-2xl transition-all duration-300 ${
                            isActive ? 'text-cream opacity-100' : 'text-cream/50 opacity-60'
                          }`}
                        >
                          {tab.label}
                        </h3>
                        <p
                          className={`font-body text-sm xl:text-base mt-2 leading-relaxed max-w-md transition-all duration-300 ${
                            isActive ? 'text-cream-soft/70 opacity-100' : 'text-cream-soft/40 opacity-60'
                          }`}
                        >
                          {tab.detail}
                        </p>
              ...
              <a
                href="#"
                className="inline-flex items-center gap-2 sm:gap-3 mt-6 sm:mt-8 px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-gold to-gold-soft text-forest-deep rounded-full font-heading font-bold uppercase tracking-widest text-xs sm:text-sm hover:shadow-sacred-glow transition-all"
              >
            ...
            <div className="flex-1 w-full relative z-10 h-[62vh] sm:h-[68vh] xl:h-[74vh] max-h-[760px]">
              {tabs.map((tab, i) => {
                ...
                return (
                  <div
                    key={tab.id}
                    className="absolute inset-0 rounded-2xl xl:rounded-3xl overflow-hidden shadow-xl border border-gold/20 will-change-transform"
                    style={{
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 xl:p-8">
                      ...
                      <h3 className="font-display text-2xl xl:text-4xl text-cream mt-1 mb-2">{tab.label}</h3>
                      <p className="text-cream-soft/70 font-body text-sm xl:text-base max-w-md">{tab.desc}</p>
                    </div>
                  </div>
                );
              })}
...
export function ChooseByIntention() {
  return (
    <section className="relative border-y border-gold/10 bg-forest">
      <SectionHeader />
